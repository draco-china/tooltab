...
const STORAGE_KEY = "tools:toml-to-json-converter:toml-text";
...
function TomlToJsonContent() {
  ...
  useEffect(() => {
    ...
    void (async () =>
      runStructuredWorker(
        { input: deferredInput, from: "toml", to: "json" },
        controller.signal,
      ))()
      .then((result) => {
        if (
    ...
    <div className="grid gap-8">
      <div className="grid items-stretch gap-6 xl:grid-cols-2" data-tool-panels>
        <div className="flex min-w-0 flex-col overflow-hidden rounded-xl border border-field-border bg-(--field-background) shadow-field">
          <div className="relative">
            <CodeEditor
              ...
              description={m[
                "common.catalogToolTomlToJsonConverterClientTomlDescription"
              ]()}
              ...
              language="toml"
              modelPath="tooltab://structured/toml-to-json-converter.toml"
              value={input}
              height={320}
            ...
            {!input ? (
              ...
              >
                {m[
                  "common.catalogToolTomlToJsonConverterClientTomlPlaceholder"
                ]()}
              </span>
            ) : null}
          </div>
        ...
        <div className="min-w-0" aria-busy={isPending}>
          {isPending ? (
            <OutputSkeleton label={m["tools.csvToJsonConverter.jsonLabel"]()} />
            ...
            <>
              <CodeBlock
                ...
                description={m[
                  "tools.tomlToJsonConverter.clientJsonDescription"
                ]()}
                language="json"
  ...
  return (
    ...
    >
      <p className="text-sm leading-6 text-muted">
        {m["tools.tomlToJsonConverter.clientJsonEmptyDescription"]()}
      </p>
    </section>
  );
}
...
function TomlToJsonArticle() {
  return (
    <ToolArticle>
      <h2>{m["tools.csvToJsonConverter.article.whatTitle"]()}</h2>
      <p>{m["tools.tomlToJsonConverter.articlePurposeBody"]()}</p>
      ...
      <ul>
        {[
          ...
          m["tools.tomlToJsonConverter.articleWatchItems2"](),
        ].map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ul>
    ...
    error.code === "precision_loss"
      ...
      : error.code === "too_large"
        ...
        : error.code === "read_failed"
          ...
          : m["tools.tomlToJsonConverter.errorsConversionFailed"]();
