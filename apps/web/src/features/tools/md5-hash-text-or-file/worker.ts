...
import { createMD5 } from "@workspace/tools/hash/md5";
import { MAX_BYTES } from "@workspace/tools/hash/input";

let hasher: Awaited<ReturnType<typeof createMD5>> | undefined;
let bytes = 0;
...
self.onmessage = async (event) => {
  try {
    ...
    if (payload.kind === "init") {
      ...
      const result = formatDigest("MD5", bytes, hasher.digest("binary"));
      hasher = undefined;
      bytes = 0;
