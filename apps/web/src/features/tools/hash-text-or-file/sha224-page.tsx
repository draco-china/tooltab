...
import { runSha224Worker, type ShaWorkerDigest } from "./worker-client";

const DEFAULT_TEXT = "Hello, browser-native world!";
const STORAGE_KEY = "tools:sha224-hash-text-or-file:text";
...
export default function Sha224HashTextOrFile() {
  ...
  useEffect(() => {
    ...
    setState({ status: "loading" });
    void runSha224Worker(file ?? debouncedText, controller.signal)
      .then((digest) => {
  ...
  return (
    <>
      <div className="grid gap-6" data-tool-panels data-sha224-hash-page>
        <ToolPanelCard>
...
function Sha224Article() {
  return (
    <ToolArticle>
      <h2>{m["tools.sha224HashTextOrFile.article.title"]()}</h2>
      <p>{m["tools.sha224HashTextOrFile.article.summary"]()}</p>
      <p>
        <strong>
      ...
      <ul>
        ...
        <li>{m["tools.sha224HashTextOrFile.article.uses1"]()}</li>
