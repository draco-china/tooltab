...
import { runSha3224Worker, type ShaWorkerDigest } from "./worker-client";
...
export default function Sha3224HashTextOrFile() {
  ...
  useEffect(() => {
    ...
    setState({ status: "loading" });
    void runSha3224Worker(file ?? debouncedText, controller.signal)
      .then((digest) => {
  ...
  return (
    <>
      <div className="grid gap-6" data-tool-panels data-sha3-224-hash-page>
        <ToolPanelCard>
          ...
          <ToolPanelCardFooter className="block">
            <ToolFilePicker
              ...
              inputTestId="sha3-224-file-input"
              onSelect={setFile}
...
function Sha3224Article() {
  return (
    <ToolArticle>
      <h2>{m["tools.sha3224HashTextOrFile.article.title"]()}</h2>
      <p>{m["tools.sha3224HashTextOrFile.article.summary"]()}</p>
      <p>
        <strong>
      ...
      <ul>
        ...
        <li>{m["tools.sha3224HashTextOrFile.article.uses1"]()}</li>
