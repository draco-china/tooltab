...
import { runSha384Worker, type ShaWorkerDigest } from "./worker-client";
...
function Sha384HashTextOrFileContent() {
  ...
  useEffect(() => {
    ...
    setState({ status: "loading" });
    void runSha384Worker(file ?? debouncedText, controller.signal)
      .then((digest) => {
  ...
  return (
    <>
      <div className="grid gap-6" data-tool-panels data-sha384-hash-page>
        <ToolPanelCard>
...
function Sha384Article() {
  return (
    <ToolArticle>
      <h2>{m["tools.sha384HashTextOrFile.article.title"]()}</h2>
      <p>{m["tools.sha384HashTextOrFile.article.summary"]()}</p>
      <p>
        <strong>
      ...
      <ul>
        ...
        <li>{m["tools.sha384HashTextOrFile.article.uses1"]()}</li>
...
export default function Sha384HashTextOrFile() {
  return (
    <ToolPage>
      <Sha384HashTextOrFileContent />
    </ToolPage>
  );
}
