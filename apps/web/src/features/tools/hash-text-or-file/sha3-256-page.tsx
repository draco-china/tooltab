...
import { runSha3256Worker, type ShaWorkerDigest } from "./worker-client";

const DEFAULT_TEXT = "Hello, browser-native world!";
const STORAGE_KEY = "tools:sha3-256-hash-text-or-file:text";
...
export default function Sha3256HashTextOrFile() {
  ...
  useEffect(() => {
    ...
    setState({ status: "loading" });
    void runSha3256Worker(file ?? debouncedText, controller.signal)
      .then((digest) => {
  ...
  return (
    <>
      <div className="grid gap-6" data-tool-panels data-sha3-256-hash-page>
        <ToolPanelCard>
          ...
          <ToolPanelCardFooter className="block">
            <ToolFilePicker
              ...
              inputTestId="sha3-256-file-input"
              onSelect={setFile}
...
function Sha3256Article() {
  return (
    <ToolArticle>
      <h2>{m["tools.sha3256HashTextOrFile.article.title"]()}</h2>
      <p>{m["tools.sha3256HashTextOrFile.article.summary"]()}</p>
      <p>
        <strong>
      ...
      <ul>
        ...
        <li>{m["tools.sha3256HashTextOrFile.article.uses1"]()}</li>
