...
import { runSha3512Worker, type ShaWorkerDigest } from "./worker-client";

const DEFAULT_TEXT = "Hello, browser-native world!";
const STORAGE_KEY = "tools:sha3-512-hash-text-or-file:text";
...
export default function Sha3512HashTextOrFile() {
  ...
  useEffect(() => {
    ...
    setState({ status: "loading" });
    void runSha3512Worker(file ?? debouncedText, controller.signal)
      .then((digest) => {
  ...
  return (
    <>
      <div className="grid gap-6" data-tool-panels data-sha3-512-hash-page>
        <ToolPanelCard>
          ...
          <ToolPanelCardFooter className="block">
            <ToolFilePicker
              ...
              inputTestId="sha3-512-file-input"
              onSelect={setFile}
...
function Sha3512Article() {
  return (
    <ToolArticle>
      <h2>{m["tools.sha3512HashTextOrFile.article.title"]()}</h2>
      <p>{m["tools.sha3512HashTextOrFile.article.summary"]()}</p>
      <p>
        <strong>
      ...
      <ul>
        ...
        <li>{m["tools.sha3512HashTextOrFile.article.uses1"]()}</li>
