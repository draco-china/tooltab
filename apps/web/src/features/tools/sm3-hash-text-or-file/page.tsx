...
const STORAGE_KEY = "tools:sm3-hash-text-or-file:text";
...
function Sm3HashContent() {
  const locale = getLocale();
  const id = useId();
  const fileError = m["tools.sm3HashTextOrFile.ui.fileError"]();
  const textError = m["tools.sm3HashTextOrFile.ui.textError"]();
  ...
  return (
    <div className="grid gap-8">
      ...
      <ToolArticle>
        <h2>{m["tools.sm3HashTextOrFile.ui.articleTitle"]()}</h2>
        <p>{m["tools.sm3HashTextOrFile.ui.articleBody"]()}</p>
        <p>
          <strong>
        ...
        <ul>
          {[
            [
              ...
              m["tools.sm3HashTextOrFile.ui.characteristics1Body"](),
            ],
            [
              m["shared.blakeHash.blake2bArticleCharacteristics3Title"](),
              m["tools.sm3HashTextOrFile.ui.characteristics2Body"](),
            ],
            [
              m["shared.blakeHash.blake2bArticleCharacteristics5Title"](),
              m["tools.sm3HashTextOrFile.ui.characteristics3Body"](),
            ],
          ].map(([title, body]) => (
            <li key={title}>
        ...
        <ul>
          {[
            m["tools.sm3HashTextOrFile.ui.uses1"](),
            m["tools.sm3HashTextOrFile.ui.uses2"](),
            m["tools.sm3HashTextOrFile.ui.uses3"](),
          ].map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
...
export default function Sm3Hash() {
  return (
    <ToolPage>
      <Sm3HashContent />
    </ToolPage>
...
async function hashSource(
  ...
  return runStreamHash("SM3", zeroingChunks(source, signal), signal);
}
