...
import type { CityPrelude } from "@workspace/tools/hash/city";
import {
  type Algorithm,
  STREAM_ALGORITHMS,
} from "@workspace/tools/hash/streaming";
import type { HashResult } from "./logic";
...
export async function runStreamHash(
  ...
  let bytes = 0;
  try {
    await send({ kind: "init", algorithm, seed, key, city });
